...
- **Wander 모드** — 커서와 무관하게 화면을 자유롭게 배회합니다. 무작위 지점으로
  ...
  Follow 모드는 커서가 30초만 멈춰 있어도 잠들지만, Wander 모드는 그냥 구경만
  하고 있어도 계속 잠들어버리지 않도록 훨씬 긴 유휴 시간(기본 5분) 동안 커서가
  멈춰 있어야 배회하던 자리에서 그대로 잠들고(역시 수면 애니메이션이 있는 팩
  한정), 커서가 다시 움직이면 곧바로 깹니다.
- **호버 리액션** — 커서를 스프라이트 위로 천천히 가져다 대면 폴짝 뛰거나,
  제자리에서 빙글빙글 돌거나, 공격 모션을 재생하는 것 중 하나가 무작위로
  나갑니다(해당 모션이 있는 팩만 — 폴짝·회전 쪽이 공격보다 조금 더 자주
  나오도록 되어 있습니다). 빠르게 스쳐 지나가는 경우는 의도적으로 무시되므로,
