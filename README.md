...
- **Wander 모드** — 커서와 무관하게 화면을 자유롭게 배회합니다. 무작위 지점으로
  ...
  Follow 모드처럼 커서가 30초 동안 멈춰 있으면 배회하던 자리에서 그대로
  잠들고(역시 수면 애니메이션이 있는 팩 한정), 커서가 다시 움직이면 곧바로
  깹니다.
- **호버 공격** — 커서를 스프라이트 위로 천천히 가져다 대면 공격 애니메이션이
  나갑니다(공격 애니메이션이 있는 팩 한정). 빠르게 스쳐 지나가는 경우는
  의도적으로 무시되므로, 지나가다가 우연히 스치기만 해도 공격이 튀어나오는
  일은 없습니다. 한 번 공격한 뒤에는 약 2초의 쿨다운이 있고, 다시 발동하려면
  커서가 스프라이트 밖으로 나갔다가 다시 들어와야 합니다. 잠든 상태에서
  커서를 갖다 대면 잠에서 깨어나며 그대로 반격합니다.
- **성장과 진화** — 포켓몬마다 자기만의 경험치(XP)를 쌓으며 자랍니다.
  - 함께 보낸 시간, 실제로 이동한 거리, 호버 공격 같은 상호작용이 모두
...
- **기분 말풍선** — 포켓몬 머리 위에 작은 초상화 말풍선이 떠서 지금 기분을
  보여줍니다. 진화를 마치면 기뻐하고, 호버 공격으로 놀아주면 행복해하고,
