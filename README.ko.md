...
- 연결된 모든 디스플레이마다 하나씩, 포커스를 절대 빼앗지 않는 **클릭 통과
  오버레이** 위에 떠서, 작업을 방해하지 않고 위를 부드럽게 떠다니고,
- **디스플레이 경계를 걸어서 넘나들며 커서를 따라다니며** (순간이동이 아니라
  실제로 걸어서 건너갑니다), 커서가 있는 모니터로 이동하고,
- wander(배회) 모드에서는 한 화면이 아니라 **데스크톱 전체를 배회**하고,
...
- **`desktop/main.cjs`** — Electron 래퍼. 디스플레이마다 오버레이 창을 하나씩
  만들고, "엔진" 역할을 맡은 디스플레이의 창에만 원본 커서 위치를(약 60Hz,
  전역 데스크톱 좌표로) 전달하고, 디스플레이 hotplug 시 창들을 재구성하고,
  트레이 메뉴를 구성하고, 설정 창을 엽니다.
- **`desktop/shim-preload.cjs`** — 엔진 창과 설정 창이 로드하는 작은 `chrome.*`
  shim. Electron IPC 위에서 `chrome.storage.sync/local`(`get`/`set`/`onChanged`)과
  `chrome.runtime`(`getURL`/`sendMessage`/`onMessage`/`id`)을 구현해, `src/popup`과
  `src/content.js`가 아무 수정 없이 동작하게 합니다. 또한 멀티 디스플레이 레이아웃
  (`window.__VCP1_WORLD__`)을 페이지에 주입하고, 엔진의 매 프레임 스프라이트
  스냅샷을 `main.cjs`로 중계해 미러 창들에 전달합니다.
- **`desktop/mirror.html` / `mirror-preload.cjs` / `mirror-render.js`** — 엔진을
  맡은 디스플레이를 제외한 나머지 디스플레이는 `overlay.html` 대신 이 파일들을
  로드합니다. 이 쪽에는 follow/wander 로직이 전혀 없습니다 — `main.cjs`가 엔진의
  매 프레임 스프라이트 스냅샷(위치, 시트, 프레임)을 브로드캐스트하면
  `mirror-render.js`는 그 위치를 자기 디스플레이의 로컬 좌표로 변환해 똑같이
  생긴 스프라이트 엘리먼트를 그리기만 합니다 — 즉 물리 시뮬레이션은 하나만
  돌고, 나머지는 그것을 수동적으로 비추는 화면일 뿐입니다.
